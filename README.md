...
- **表示モード** — グリッド（L / M / S）・リスト・コンパクトの5段階をタグバーから切替
- **チャンネル検索** — ステータスバーの検索欄から YouTube + Twitch を同時検索、直接チャンネル登録可
- **テーマプリセット** — Dark / Midnight / Slate / Light を設定から切替
- **アクセントカラー** — カラーピッカーで UI のアクセント色を変更
- **サイドバーリサイズ** — 境界をドラッグして幅を変更（140〜320px）
- **サイドバー / ヘッダー表示切替** — ‹ / › ボタン、▴ / ▾ ボタンでワンクリック非表示・復元
- チャンネルアイコン・名前クリックでチャンネルページへ遷移
- 動画タイトルクリックで動画ページへ遷移（コンパクトモードでサムネイルが非表示でも開ける）
- チャンネル ID URL（`/channel/UCxxx`）でのチャンネル登録時にアイコンを正しく取得するよう修正
...
タグバー右端のモードボタンをクリックするたびに表示が切り替わります。ボタンには次のモードのアイコンが表示されます。
...
ステータスバーの検索欄にチャンネル名を入力して Enter を押すと、YouTube と Twitch を同時に検索します。

- **＋** ボタンを押すと URL が入力済みの登録画面が開き、そのまま登録できます
- 登録済みのチャンネルは「登録済み」バッジで区別されます
- 残り検索回数がステータスバーに表示されます（デフォルト 10回/日、設定で最大 90回まで変更可）
- YouTube 検索は 1回あたり 100 ユニットのクォータを消費します

### 外観カスタマイズ

⚙設定の「表示カスタマイズ」から変更できます。

| 項目 | 説明 |
|---|---|
| テーマ | Dark / Midnight / Slate / Light から選択 |
| アクセントカラー | ボタン・フォーカス色などの UI 強調色を変更 |

### サイドバー / ヘッダー操作

| 操作 | 方法 |
|---|---|
| サイドバー幅変更 | サイドバー右端の境界をドラッグ（140〜320px） |
| サイドバー非表示 | サイドバーヘッダーの **‹** ボタン |
| サイドバー復元 | コンテンツ左端に現れる **›** タブ |
| ヘッダー非表示 | ステータスバー右端の **▴** ボタン |
| ヘッダー復元 | 画面右上に現れる **▾** タブ |
...
| 📊 | API 使用量・成功率の統計を表示 |
| ⚙ | API キー・タグ・メンバー限定キーワード・ショートキーワード・外観などの設定 |
| ▴ | ヘッダーを非表示 |
...
| `search_max_daily` | `10` | チャンネル検索の1日あたりの上限回数（1〜90）。YouTube 検索は 1回 100 ユニット消費。YouTube API キーがない場合はカウントされない |
...
| GET | `/api/search/channels?q=` | チャンネル検索（YouTube + Twitch）。残り回数・登録済みフラグ付き |
| GET | `/api/tags` | タグ一覧 |
